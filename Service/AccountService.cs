    ...
    public class AccountService
    {
...

        {
            ...
            var transaction = new TransactionHistory
            {
                Amount = amount,
                Date = DateTime.Now,
                Description = $"Withdrew from: {target}"
            };
            transactionHistory.Add(transaction);
            Console.WriteLine("Enter 1 to go back to menu or enter 0 to exit");
            var next = Console.ReadLine() ;
            if (next == "1")
            {
                Menu.BankMenu(customer);
        ...
        public static void StatementOfAccount(Customer customer)
        {
            ...
            var table = new ConsoleTable("Account Number", "Date", "Amount", "Description");

            foreach (var transaction in transactionHistory)
            {
                table.AddRow(account.AccountNumber, transaction.Date, transaction.Amount, transaction.Description);
            }
